(function () {
  const init = () => {
    ...
    if (requiredElements.some((element) => !element)) {
      console.warn('Prepify mock builder: missing required markup.');
      return;
    }
    ...
    const applyQueryParams = () => {
      ...
      const categoryValues = params
        .getAll('categories')
        .filter((value) => categoryPool.some((category) => category.id === value));
      ...
      if (difficultyParam) {
        const match = Array.from(form.elements.difficulty || []).find(
          (radio) => radio.value === difficultyParam
        );
        if (match) {
          match.checked = true;
        }
      ...
      if (Number.isFinite(countParam)) {
        const min = Number(countRange.min) || 5;
        const max = Number(countRange.max) || 50;
        const clamped = Math.min(Math.max(countParam, min), max);
        countRange.value = String(clamped);
        countDisplay.textContent = String(clamped);
      }

      const timeParam = Number.parseInt(params.get('time'), 10);
      if (Number.isFinite(timeParam)) {
        const minTime = Number(customTime.min) || 5;
        const maxTime = Number(customTime.max) || 120;
        const clampedTime = Math.min(Math.max(timeParam, minTime), maxTime);
        customTime.value = String(clampedTime);
      ...
      if (categoryValues.length || difficultyParam || Number.isFinite(countParam) || Number.isFinite(timeParam) || seedParam) {
        builderStatus.textContent = 'Loaded mock settings from the URL.';
      }
